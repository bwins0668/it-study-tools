    ...
    public partial class ThisAddIn
    {
        ...
        private void ThisAddIn_Startup(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                _pane.Height = 140;
                _pane.Visible = true;
                _probe.Write("pane.visible", _sessionId.ToString("N"), excelPid: Process.GetCurrentProcess().Id, paneTitle: "MOS Native Exam Host · R3 VSTO POC", dockPosition: "Bottom", paneVisible: true);
            ...
            try
            {
                ...
                if (result.Ok)
                {
                    ...
                    if (!string.IsNullOrEmpty(result.TaskId))
                    {
                        ...
                        _paneControl.OnScoreClicked = () => HandleScore(pid);
                    }
                }
                else
                {
        ...
        private void HandleCompletion(int pid)
        {
            ...
            try
            {
                ...
                if (result.Ok) { _probe?.Write("training.complete.accepted", guid, excelPid: pid); _paneControl.ShowCompletionAccepted(); }
                else { _probe?.Write("training.complete.rejected", guid, excelPid: pid); }
            }
            catch (Exception ex) { _probe?.Write("training.complete.http_failed", guid, excelPid: pid); Debug.WriteLine("Completion failed: " + ex.Message); }
        }

        private void HandleScore(int pid)
        {
            var guid = _sessionId.ToString("N");
            ...
            try
            {
                ...
                if (result.Ok)
                {
                    ...
                    _paneControl.ShowScoreResult(result.ResultJa, result.ResultZh, result.Earned > 0);
                }
                else { _probe?.Write("scoring.request.rejected", guid, excelPid: pid); _paneControl.ShowScoreResult("评分失败", "评分失败", false); }
            }
            catch (Exception ex) { _probe?.Write("scoring.http_failed", guid, excelPid: pid); Debug.WriteLine("Score failed: " + ex.Message); }
        }
