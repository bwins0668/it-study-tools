-- ============================================================
-- Study Tools — Supabase DB Init Draft (Round 17.1)
--
-- Purpose:  DDL draft for sync tables. NOT executed yet.
-- Schema:   public
-- Auth:     Supabase auth.users referenced via foreign key
-- Security: Row-Level Security (RLS) policies included as comments
-- ============================================================

-- Extensions
CREATE EXTENSION IF NOT EXISTS "pgcrypto";   -- gen_random_uuid()
CREATE EXTENSION IF NOT EXISTS "pg_stat_statements";  -- optional monitoring
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sync_pull_progress
  ON public.learning_progress(user_id, updated_at)
  WHERE deleted_at IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sync_pull_translations
  ON public.user_translations(user_id, updated_at)
  WHERE deleted_at IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sync_pull_bookmarks
...
--    auth.users(id).
--
-- 2. RLS policies are commented out — enable them before going
--    to production. Every table has the same policy:
--    "user can only see/modify their own rows WHERE auth.uid() = user_id"
--
-- 3. No real project URL, API key, or secret is stored in this file.
--    Supabase project URL and anon key go to .env.local (gitignored).
--
